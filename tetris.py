...
class Game:
    ...
    def __init__(self):
        ...
        """
        # T ODO start() needs to be refactored so that the creation of the
    ...
    def timer(self):
        ...
        if not self.current_shape.fall():
            ...
            if self.is_game_over():
                # T ODO This is a problem. You rely on the timer method to
    ...
    def remove_complete_lines(self):
        ...
        # T ODO Would be cooler if the line flashed or something
...
class Shape:
    ...
    SHAPES = (("yellow", (0, 0), (1, 0), (0, 1), (1, 1)),  # square
    ...
    def __init__(self, canvas):
        ...
        for point in self.shape[1:]:
            box = canvas.create_rectangle(point[0] * Shape.BOX_SIZE + Shape.START_POINT, point[1] * Shape.BOX_SIZE,
                                          point[0] * Shape.BOX_SIZE + Shape.BOX_SIZE + Shape.START_POINT,
                                          point[1] * Shape.BOX_SIZE + Shape.BOX_SIZE, fill=self.color)
    ...
    def rotate(self):
        ...
        def get_move_coords(box_inp):
            """Return (x, y) boxes needed to rotate a box around the pivot."""
            box_coords = self.canvas.coords(box_inp)
            pivot_coords = self.canvas.coords(pivot)
            x_diff = box_coords[0] - pivot_coords[0]
            y_diff = box_coords[1] - pivot_coords[1]
            x_move_loc = (- x_diff - y_diff) / self.BOX_SIZE
            y_move_loc = (x_diff - y_diff) / self.BOX_SIZE
            return x_move_loc, y_move_loc
    ...
    def can_move_box(self, box, x, y):
        ...
        overlap = set(self.canvas.find_overlapping((coords[0] + coords[2]) / 2 + x, (coords[1] + coords[3]) / 2 + y,
                                                   (coords[0] + coords[2]) / 2 + x, (coords[1] + coords[3]) / 2 + y))
